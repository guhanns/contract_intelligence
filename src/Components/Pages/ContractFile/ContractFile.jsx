    ...
    useEffect(()=>{
      ...
      const extractContracts = async () => {
        ...
        for (const li of location?.state?.uploadResults) {
          ...
          try {
            await axios.post(`${NodeURL}/icontract/text_extractor/process_contract`, formData);
          } catch (err) {
                ...
                <tbody>
                  ...
                  {isExtracting &&
                    ...
                    location?.state?.files?.map((li) => (
                      <tr className="contract-result-list" key={li?.file?.name}>
                  ...
                  {/* Only show loading if there’s nothing yet */}
                  {contractList?.length === 0 && (
                    <tr>
